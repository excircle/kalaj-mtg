...
export default function Green() {
  ...
  useEffect(() => {
    async function fetchGreen() {
      try {
        ...
        console.error(err)
        setError('Failed to load green cards')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading) return <p className="p-4">Loading green cards…</p>
  if (error)   return <p className="p-4 text-red-600">{error}</p>
  if (cards.length === 0) return <p className="p-4">No green cards found.</p>
