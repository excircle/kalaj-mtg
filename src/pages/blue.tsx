...
export default function Blue() {
  ...
  useEffect(() => {
    async function fetchBlue() {
      try {
        ...
        console.error(err)
        setError('Failed to load blue cards')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading) return <p className="p-4">Loading blue cards…</p>
  if (error)   return <p className="p-4 text-red-600">{error}</p>
  if (cards.length === 0) return <p className="p-4">No blue cards found.</p>
