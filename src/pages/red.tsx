...
export default function Red() {
  ...
  useEffect(() => {
    async function fetchRed() {
      try {
        ...
        console.error(err)
        setError('Failed to load red cards')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading) return <p className="p-4">Loading red cards…</p>
  if (error)   return <p className="p-4 text-red-600">{error}</p>
  if (cards.length === 0) return <p className="p-4">No red cards found.</p>
